...
if(!host || !key || !secret) {
  sys.error("Missing config parameters. Config is "+JSON.stringify(config, null, " "));
  process.exit();
}
...
var server = http.createServer(function (request, response) {
  ...
  // ...
    ...
    response.end('<html> tyo-stream <b>'+info.host+'</b> wants to access your Twitter account.</b><br>'+
  ...
  /*
   * the server side compilation caused: 
   * TypeError: Cannot read property 'contents' of undefined
   * 
   */
//  else if(url.pathname.match(/css\/\w+\.less/)) {
...
//  }
  ...
  // All other requests
    ...
    /* 
     * initially, the static server is not responding to static file request
     * 
     * bascially, we need to "resume()" it to make it reponse to client
     */
    else {
